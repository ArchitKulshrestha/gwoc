...
const Navbar = () => {
  ...
  return (
    <motion.nav
      className="fixed px-6 lg:px-20 z-50 backdrop-blur-[2px]  bg-background-100/60 w-full h-[70px] flex justify-between items-center border-b-2 border-background-100 shadow-md"
      initial={{ y: -100 }}
      ...
      <ul className="hidden lg:flex h-full flex-row gap-4 items-center">
        {NavLinks.map((link) => (
          <li
            className="  text-text-700 hover:text-text-800 hover:scale-105 transition-all duration-200"
            key={link.id}
            id={link.id}>
            <Link
